...
def convert_angleSL_to_psiiota(phis, thetas, phil, thetal):

    ra = phis
    dec = np.pi/2 - thetas

    cosinc = np.cos(thetal)*np.sin(dec) + np.sin(thetal)*np.cos(dec)*np.cos(phil - ra)
    tanpsi_num = (np.cos(thetal) + cosinc*np.sin(dec)) 
    tanpsi_denom = np.cos(dec)*np.sin(thetal)*np.sin(phil - ra)
    inc = np.arccos(cosinc)
    psi = np.arctan2(tanpsi_num,tanpsi_denom)
    return ra, dec, inc, psi
